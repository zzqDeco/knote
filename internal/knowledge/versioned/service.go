...
type Service interface {
	...
	Versions(ctx context.Context, limit int) ([]repository.Version, error)
	Commit(ctx context.Context, message string) (repository.CommitResult, error)
	Release(ctx context.Context, tag string) error
	Checkout(ctx context.Context, ref string, opts repository.CheckoutOptions) error
	Status(ctx context.Context) (repository.Status, error)
	Mode() Mode
}
...
func (s service) Status(ctx context.Context) (repository.Status, error) {
	versions, err := s.requireVersions()
	if err != nil {
		return repository.Status{}, err
	}
	return versions.Status(ctx)
}

func (s service) Diff(ctx context.Context, ref string) (string, error) {
	versions, err := s.requireVersions()
	if err != nil {
		return "", err
	}
	return versions.Diff(ctx, ref)
}

func (s service) Versions(ctx context.Context, limit int) ([]repository.Version, error) {
	versions, err := s.requireVersions()
	if err != nil {
		return nil, err
	}
	return versions.Versions(ctx, limit)
}

func (s service) Commit(ctx context.Context, message string) (repository.CommitResult, error) {
	versions, err := s.requireVersions()
	if err != nil {
		return repository.CommitResult{}, err
	}
	return versions.Commit(ctx, message)
}
...
func (s service) Release(ctx context.Context, tag string) error {
	...
	status, err := versions.Status(ctx)
	if err != nil {
		return err
	}
	if status.Dirty {
	...
	if err := s.repo.EvalGate(ctx); err != nil {
		return err
	}
	return versions.Tag(ctx, tag)
}

func (s service) Checkout(ctx context.Context, ref string, opts repository.CheckoutOptions) error {
	versions, err := s.requireVersions()
	if err != nil {
		return err
	}
	return versions.Checkout(ctx, ref, opts)
}

func (s service) requireVersions() (repository.Versions, error) {
	if s.versions == nil {
		return nil, fmt.Errorf("version repository is not configured")
	}
