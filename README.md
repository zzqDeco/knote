...
`KNOTE_EINO_MODEL_PROFILE` selects a profile from `.knote/config.yaml` and defaults to `default`. Environment variables override the selected profile. `OPENAI_MODEL`, `OPENAI_API_KEY`, and `OPENAI_BASE_URL` are also accepted; when those overrides are present, provider defaults to `openai-compatible`. `KNOTE_EINO_REASONING_EFFORT` accepts `low`, `medium`, or `high`. `KNOTE_TURN_TIMEOUT` accepts a positive Go duration and defaults to `3m`; it bounds each message or confirmation turn. Cancellation and terminal records receive a detached persistence window no longer than the configured timeout and capped at two seconds. `KNOTE_RUNTIME_MODE=direct` is rejected.
