...
if [[ -z "${KNOTE_EINO_API_KEY:-}" ]]; then
  CONFIG_PATH="${KNOTE_CLIPROXY_CONFIG:-/Users/zhaoziqian/CLIProxyAPI/config.yaml}"
  if [[ -f "${CONFIG_PATH}" ]] && command -v ruby >/dev/null 2>&1; then
    KNOTE_EINO_API_KEY="$(ruby -ryaml -e 'cfg=YAML.load_file(ARGV[0]); print Array(cfg["api-keys"]).first.to_s' "${CONFIG_PATH}")"
