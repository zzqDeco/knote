...
Document is the Phase 1 ACL boundary. Every resource handle carries the stable identity of its authorization boundary. Chunk inherits its Document and every Chunk decision must match both that immutable parent Document identity and the exact parent Document handle used as the authorization resource, including the inherited ACL version; a Chunk-scoped, stale, or unrelated Document authorization object is invalid. Every Entity handle, including an Entity nested in another item's provenance, requires at least one authorized Document or Chunk in that same individual support and cannot borrow grounding from another support or establish itself by self-support. Claim authorization is independent from Entity visibility. A denied Claim or frontier item cannot participate in a later graph hop.
